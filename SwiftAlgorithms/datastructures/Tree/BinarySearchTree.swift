...
extension BinarySearchTree {
    // raywenderlich code
    // 내가 짠 코드는 이미 구현된 computed property 를 사용해서 좀 장황하게...
    // >> 아직 optional chaining 에 익숙하지 않아서 아래와 같은 코드가 한 번에 떠오르지 않는거 같다.
...
//                if child!.hasLeft {
//                    print("\(child!.value).hasLeft == true")
//                    child = child!.left
//                    if value == child!.value {
//                        print("find1!!(right)")
//                        break
...
//                print("\(value) >= \(child!.value)")
//                if child!.hasRight {
//                    print("\(child!.value).hasRight == true")
//                    child = child!.right
//                    if value == child!.value {
//                        print("find2!!(right)")
//                        break
...
//        print("<<< end of \(#function)")
    ...
    /*
     ...
     tree.insert(7)
     >> recursive ways
      > if 7 < 5 = false then else
      > if hasRight = true then *right.insert(7)
       > if 7 < 10 = true
       > if hasLeft = true the **left.insert(7)
        > if 7 < 8 = true
        > if hasLeft = false the left = newNode[7]
        > newNode[7].parent = node[8] > go to ** point. have no rest operations > go to * point. have no rest operations > end of method
     */
}

...
public final class LinkedList3<T> {
    // - Mark: LinkedListNode
    // node
    ...
    public class LinkedListNode3<T> {
        public var value: T
        public var next: LinkedListNode3?
        weak public var previous: LinkedListNode3?
        init(_ value: T) {
            self.value = value
        }
    }
    ...
    public typealias Node = LinkedListNode3<T>
    ...
    public var last: Node? {
        ...
        guard var node = head else {
            print("\(#function) list[LinkedList3: (\(self))] has no node[LinkedListNode3]")
            return nil
        }
        while let next = node.next {
            node = next
        }
//        print("\(#function) last: \(node.value)")
        return node
    }
    ...
    public func node(at index: Int) -> Node {
        assert(!isEmpty(), "list[LinkedList3: (\(self))] has no node[LinkedListNode3]")
        if index == 0 {
            return head!
        } else {
            var node = head
...
//                print("\(#function) searching loop: node = \(node!.value)")
    ...
    public func append(_ value: T) {
        let newNode = LinkedListNode3(value)
        append(newNode)
    }
...
//            print("\(#function) former tail: \(tail!.value)")
    ...
    public func append(_ list: LinkedList3) {
        if isEmpty() {
            head = list.head
            tail = list.tail
...
//            print("\(#function) tail = \(tail!.value)")
    ...
    public func insert(_ list: LinkedList3, at index: Int) {
        if index == 0 {
...
extension LinkedList3: CustomStringConvertible {
    ...
    public func map<U>(transform: (T) -> U) -> LinkedList3<U> {
        let result = LinkedList3<U>()
        var node = head
        while node != nil {
    ...
    public func filter(predicate: (T) -> Bool) -> LinkedList3<T> {
        let result = LinkedList3<T>()
        var node = head
        while node != nil {
